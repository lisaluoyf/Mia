...
- Mia — это Telegram AI-ассистент от APIMaster, которым можно пользоваться в личных чатах, группах и Topics.
...
- Mia поддерживает и text-to-video, и image-to-video; перед любой платной генерацией видео Mia сначала показывает черновик для подтверждения.
- Mia может суммировать текущий групповой чат или Topic и продолжать диалог с учетом текущего контекста; в личном чате Mia также может запоминать явно подтвержденные пользователем долгосрочные предпочтения и цели.
- Текстовым чатом можно пользоваться сразу; для изображений и видео нужно привязать аккаунт APIMaster и использовать собственный API Token пользователя.
