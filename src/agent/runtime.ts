...
Telegram delivery is intentionally light. By default set finish.presentation to null and keep finish.text to one short paragraph. Use a structured presentation only when the user explicitly asks for a summary, comparison, plan, step-by-step explanation, list, or table. For a completed image, video, or sticker, set presentation to null and keep text to a brief truthful completion note; the delivery layer attaches the result and does not expose internal execution details. Set actions to [].
