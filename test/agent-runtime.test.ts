...
describe("goal-driven agent runtime", () => {
  it("accepts provider-owned search source IDs after an observed hosted web search", async () => {
    const search = { type: "web_search_call", id: "ws_1", action: { type: "search", query: "latest news" } };
    const finishSearch = () => call("finish", { text: "Current answer", status: "completed", requirements: [{ requirement: "Current facts", kind: "search", satisfied: true, evidence: ["turn1view5", "turn1reddit3"] }] }, "finish-search");
  ...
  it("does not accept a search claim without a hosted web search result", async () => {
    const f = setup(() => call("finish", { text: "Current answer", status: "completed", requirements: [{ requirement: "Current facts", kind: "search", satisfied: true, evidence: [] }] }, "finish-search"));
    f.runtime.enqueue(input); await f.runtime.drain();
