...
- Telegram 中途补充会使旧模型步骤失效；取消阻止后续操作；视频及额外付费生成需确认。媒体结果与 Telegram 交付分离，未知提交或未知发送不盲目重试。
- 复用现有上下文、本人凭证、群媒体权限和媒体 worker，接入看图、生图／改图、贴纸、视频、上下文读取及可选搜索。未引入 Skill，未改生产模型配置。
- 本地验证：`pnpm test` 62 个文件、325 项测试通过；`pnpm typecheck`、`pnpm lint`、`pnpm build` 和 `git diff --check` 通过。覆盖循环、协议、真实 SQLite 重启、媒体 worker、Telegram 入口与交付恢复；尚未执行真实付费生成或部署。
- 第一版明确限制：单进程；每用户／聊天／话题一个活动目标；主模型需支持 Responses 原生函数调用；任务内超长上下文先暂停；尚未接入长期记忆写回、金额预算与完整 legacy 分享展示。详细范围及开关见 README 与设计草案的“当前实现状态”。

### Agent Loop 重设计调研（2026-09-07）

- 已对照本地 Codex 开源 runtime、Claude Code 官方 agentic loop 文档、用户提供的二手源码解析，以及 Mia 当前主链路，形成 [Agent Loop 重设计草案](agent-loop-design.md)。
- 建议保留现有媒体 job、视频确认、权限、上下文和展示能力，增加可恢复的单 agent runtime；先做执行记录，再接工具往返、异步续跑和中途修改。
- 状态：调研与设计草案，尚未实现、测试或部署。主循环模型选择、自动重生成预算和任务关联体验仍需确认。
- 验证范围：本地源码与官方文档交叉核对、文档结构和引用检查；不表示新 runtime 已通过功能测试，亦未重新核实生产版本。
...
- [x] Telegram 账号双路径与社群解耦（2026-09-06）：Mia 未绑定媒体请求现在同时提供“用 Telegram 创建/登录”和“我已有 APIMaster 账号”。前者严格按已验证 TG 身份登录或创建 TG-only 账号，并忽略无关的网站登录态；后者保留一次性授权直到用户登录并明确确认绑定。两个已经存在的 APIMaster UID 不自动合并，也不转移余额、订阅、API Key、订单或体验状态。账号登录绑定以 APIMaster `user_social_bindings` 为准，new-api `users.telegram_id` 只做账号镜像；Telegram 入群验证只维护社群状态，清除社群验证不会再解绑账号。新建账号或首次绑定后会发送可选的 APIMaster 社群邀请，入群不是 Mia 正常使用的前置条件。
...
- Telegram 双路径登录与社群状态拆分：Mia `8b806d9`、APIMaster `d7af4a8`、new-api `1cda080` 已按精确 GitHub SHA 发布（2026-09-06）。本地验证为 Mia 273/273、APIMaster 认证策略 11/11、new-api `model`/`controller` 测试通过；三端类型检查、改动范围 Lint、生产构建和 `git diff --check` 通过，new-api 前端全量 Lint 为 0 error/19 个既有 warning。生产 new-api 完成 GHCR 蓝绿切换并通过公网冒烟，APIMaster 两个 PM2 worker 在线且完整只读冒烟 21/21，两个深链入口均返回 `307` 并指向 `t.me`；Mia 发布到 `/srv/mia/releases/mia-git-8b806d9b511f-20260906T100117Z`，耗时 8 秒，内部健康检查 `ok`、PM2 `online`/0 重启，回滚目标为 `mia-git-a58eb3314416-20260906T022524Z`。尚未修改任何真实用户数据，仍待用户在真实 Telegram 客户端分别验收“新建/登录 TG 账号”和“绑定已有 APIMaster 账号”。
- Mia 媒体模型权限反馈修复：用户已绑定且已有可用 Token、但所选媒体模型暂时不可访问时，new-api 不再返回与“没有 Token”相同的 `no_usable_api_key`，改为 `selected_model_unavailable`；Mia 对应提示稍后重试或更换模型，不再误导用户创建新 Token。new-api `7163b50` 已通过 GHCR 精确 SHA 蓝绿发布，Mia `c525009` 已原子发布；Mia 275/275、类型检查、Lint、生产构建、new-api `model/controller` 测试和 `git diff --check` 均通过。生产同一开发者 TG 账号只读验证 `gpt-image-2` 与 `MiniMax-H3` 均返回 HTTP 200，无效模型返回 HTTP 404 + `selected_model_unavailable`；Mia `/health` 正常、PM2 `online`/0 重启，APIMaster 只读生产冒烟 21/21 通过（2026-09-06）。
