...
if grep -q 'BEGIN MIA DE-ROMA ROUTES' "$caddyfile"; then
  echo "Mia de-roma Caddy route is already installed."
  exit 0
fi
...
echo "Mia de-roma Caddy route installed; backup=$backup"
